...
def test_write_multiomics_honors_recipe_options_in_standard_mode(tmp_path: Path):
    ...
    assert "gene_copy_number.parquet" not in {p.name for p in outputs}
    ...
    methylation = pl.read_parquet(cohort / "methylation_beta.parquet")
    assert methylation.height == 1
    assert methylation.row(0, named=True)["probe_id"] == "cg00000029"
    assert (cohort / "copy_number_segments.parquet").exists()
...
def test_standard_pipeline_applies_multiomics_recipe_options(tmp_path: Path, monkeypatch):
    ...
    rna = pl.read_parquet(tmp_path / "standard_options" / "rna_expression.parquet")
    assert rna.columns == [
        "case_id",
        "submitter_id",
        "file_id",
